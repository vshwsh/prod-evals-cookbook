...
def seed_postgres():
    ...
    try:
        ...
        print(f"   ✓ Loaded: {customer_count} customers, {refund_count} refunds, {employee_count} employees")
...
def verify_mcp_fixtures():
    ...
    if slack_path.exists():
        ...
        message_count = sum(len(m.get("thread", [])) for m in slack_data.get("messages", []))
